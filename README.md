...
| ツール | 説明 |
|---|---|
| `get_law` | e-Gov法令APIから条文を取得。法令名 + 条番号で指定。項・号・号の下のイロハ、附則にも対応。law_id 直接指定も可（例: `405AC0000000088`）|
...
所得税法、法人税法、消費税法、相続税法、租税特別措置法、国税通則法、国税徴収法、各施行令・施行規則、民法、保険法 等（26法令）
...
→ `get_law(law_name="相続税法", article="12", item=6, subitem="イ")`

`paragraph` を省いても全項から号を探します。号番号は項をまたいで重複することがあるため、
実際にどの項に一致したかを見出しに表示します。枝番号の号は `item="12の5の2"` のように
文字列で指定してください。
...
→ `get_law(law_name="保険法", format="suppl")`

> 「保険法の施行期日は？」

→ `get_law(law_name="保険法", supplementary=true, article="1")`

改正法の附則は法令番号で指定します。漢数字・算用数字のどちらの表記でも構いません。

→ `get_law(law_name="保険法", supplementary="平成29年法律第45号")`
